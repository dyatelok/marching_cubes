...
impl Runner<Building> {
    ...
    pub fn add_camera(&mut self, cam: Box<dyn Fn(f32) -> Camera3>) {
        self.camera = cam;
    }
...
impl Runner<Ready> {
    pub fn run(self) {
        ...
        while !rl.window_should_close() {
            ...
            camera = (*self.camera)(tau);
            tau += dtau;

            for line2 in self.lines.iter().map(|line3| camera.project_line(line3)) {
                d.draw_line(
